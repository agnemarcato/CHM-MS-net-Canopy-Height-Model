...
class MS_Net(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        Description
        ___________
        The function defines the training step for the model.
        ...
        batch : dict
            a dictionary holding the training data
        batch_idx : int
        ...
        loss : tensor
            the calculated loss for the current batch
        
        """
        sample, masks, xy = batch['train']
        x,y    = xy[0], xy[1]
        # training data is unpacked from batch dictionary (x module input, y is the ground truth)
        y_pred = self(x, masks)
        # model performs a forward pass using input x and masks, y_pred is list of predictions
        loss   = 0
        y_var  = 1 # torch.max(y[-1].var(), EPS)
        # could be used to normalize the loss based on the variance of the ground truth y
        for scale, [y_hat,yi] in enumerate(zip(y_pred, y)):
            loss_s = F.mse_loss(y_hat,yi)/y_var
        # the predicted output y_hat and the ground truth yi are paired together and MSE is calculated at each scale
        # loss at each scale and sum of the loses
            if torch.isnan(loss_s) or loss_s>MAXEPS:
                print(f'The loss at scale {scale} is {loss_s}')
                print('RUN')
                print('-'*100)
            # if the loss is NaN or exceeds MAXEPS, then it triggers a warning
            else:
                loss  += loss_s
            self.log(f"loss_scale{scale}", loss_s, on_step=False, on_epoch=True, logger=True, rank_zero_only=True)
            # if the loss is valid, it's added to the total loss for current batch scale
        if not loss:
            loss = None
            self.log("loss", loss_s, on_step=True, on_epoch=True, prog_bar=True, logger=True, rank_zero_only=True)
            # if loss remains 0 (or not properly calculated), it is logged and appears on progress bar
        else:
            self.log("loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True, rank_zero_only=True)
            # logs the final loss computed across the scales (every training step and averaged at the end of the epoch)
        self.log(f"loss_scale_fine", loss_s, on_step=False, on_epoch=True, logger=True, rank_zero_only=True)
        # logs the fine-scale loss across the scales (averaged at the end of epoch)
        # log files are for tensorboard
        return loss
